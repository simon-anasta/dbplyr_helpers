...
if(is.na(DEFAULT_SERVER) | nchar(DEFAULT_SERVER) < 1 |
   is.na(DEFAULT_DATABASE) | nchar(DEFAULT_DATABASE) < 1)
  stop("Default server and database must be set in utility_functions.R")

# library
library("DBI", lib.loc="~/My-R-User-Libraries")
library(dplyr)
library(dbplyr)
...
create_access_point <- function(db_connection, db, schema, tbl_name){
  ...
  # table in connection
  # assert(tbl_name %in% DBI::dbListTables(db_connection),sprintf("%s table is not found in database", tbl_name))
...
union_all <- function(table_a,table_b, list_of_columns){
  ...
  table_a <- table_a %>% dplyr::ungroup() %>% dplyr::select(list_of_columns)
  table_b <- table_b %>% dplyr::ungroup() %>% dplyr::select(list_of_columns)
  
  sql_query <- dbplyr::build_sql(con = db_connection,
...
delete_table <- function(db_connection, db, schema, tbl_name, mode = "table"){
  ...
  removal_query <- glue::glue("IF OBJECT_ID('{maybe_db_schema}.{tbl_name}', '{code}') IS NOT NULL\n",
...
create_clustered_index <- function(db_connection, db, schema, tbl_name, cluster_columns){
  warn_if_missing_delimiters(db, schema, tbl_name)
  # table in connection
  assert(tbl_name %in% DBI::dbListTables(db_connection),"table is not found in database")
  # columns are in table
  assert(all(cluster_columns %in% colnames(create_access_point(db_connection, db, schema, tbl_name))), 
...
append_database_table <- function(db_connection, db, schema, tbl_name, list_of_columns, table_to_append){
  ...
  # table in connection
  assert(remove_delimiters(tbl_name, "[]") %in% DBI::dbListTables(db_connection),"table is not found in database")
  ...
  assert(all(list_of_columns %in% colnames(create_access_point(db_connection, db, schema, tbl_name))), 
         "database table does not have the required columns")
  
  table_to_append <- table_to_append %>% dplyr::ungroup() %>% dplyr::select(list_of_columns)
...
create_view <- function(tbl_name, db_connection, db, schema, view_name, OVERWRITE = FALSE){
  ...
  # SQL query
  sql_query <- glue::glue("CREATE VIEW {schema}.{view_name} AS\n",
...
copy_r_to_sql <- function(db_connection, db, schema, sql_table_name, r_table_name, OVERWRITE = FALSE){
  ...
  suppressMessages( # mutes translation message
    DBI::dbWriteTable(db_connection,
                      DBI::Id(catalog = remove_delimiters(db, "[]"),
...
pivot_table <- function(input_tbl, label_column, value_column, aggregator = "SUM"){
  ...
  sql_query <- glue::glue("SELECT {non_pivot_columns}, \n{pivot_columns}\n",
                          "FROM (\n",
                          "{dbplyr::sql_render(input_tbl)}\n",
                          ") {from_id}\n",
                          "PIVOT (\n",
...
# 
# # connect
# > my_table = create_access_point(db_connection =  db_con, schema =  our_schema, tbl_name = "example_table")
# 
# # use
# > my_table = my_table %>%
